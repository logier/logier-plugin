...
// 抽取塔罗牌函数
function drawTarotCard(tarot) {
  let keys = Object.keys(tarot[0].cards);
  let randomIndex = Math.floor(Math.random() * keys.length);
  let randomKey = keys[randomIndex];
  let 抽到的塔罗牌数据 = tarot[0].cards[randomKey];

  let imageurl = `https://gitee.com/logier/logier-plugin/raw/master/resources/%E5%A1%94%E7%BD%97%E7%89%8C/${抽到的塔罗牌数据.type}/${抽到的塔罗牌数据.pic}.webp`;
  var options = [`正位: ${抽到的塔罗牌数据.meaning.up}`, `逆位: ${抽到的塔罗牌数据.meaning.down}`];
  var 正位还是逆位 = options[Math.floor(Math.random() * options.length)];
  var selectedOption = 正位还是逆位.split(': ');
  var position = selectedOption[0]; // 正位 或 逆位
  var meaning = selectedOption[1]; // 对应的含义

  return { 抽到的塔罗牌数据, imageurl, position, meaning };
}

//调用ai函数
async function tarotReading(apikey,占卜内容, 抽到的塔罗牌数据, 正位还是逆位) {
  const 内容 = `我请求你担任塔罗占卜师的角色。 我想占卜的内容是${占卜内容}，我抽到的牌是${抽到的塔罗牌数据.name_cn}，并且是${正位还是逆位}，请您结合我想占卜的内容来解释含义,话语尽可能简洁。`;
